...
import { listAdminStudies, type AdminStudyListItem } from "@/lib/admin";

export default function AdminStudiesTable() {
  const [items, setItems] = useState<AdminStudyListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await listAdminStudies({
        limit: 100,
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        />
        <p className="text-sm text-muted-foreground">{total} study/studies</p>
      </div>
            ...
            <tbody>
              {items.map((row) => (
                <tr key={row.study_id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{row.user_email}</td>
                  ...
                  <td className="px-4 py-3">
                    <Link
                      href={`/admin/studies/${row.study_id}`}
